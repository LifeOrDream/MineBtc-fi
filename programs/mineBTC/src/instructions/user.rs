...
/// One-time rescue claim for legacy manual round bets whose cycle
/// `UserFactionWarBets` PDA was already closed by the pre-upgrade war-claim
/// path. Normal post-upgrade claims must use `claim_round_rewards`; this path
/// is gated to sidecar-less legacy rounds from already-settled wars and returns
/// the closed `UserGameBet` rent to the original user.
pub fn legacy_claim_round_rewards_internal(
    round_id: u64,
    ctx: Context<LegacyClaimRoundRewards>,
) -> Result<()> {
    crate::log_fn!("user", "legacy_claim_round_rewards_internal");

    require!(
        ctx.accounts.round_close_state.owner != &crate::ID
            && ctx.accounts.user_game_bet_close_state.owner != &crate::ID,
        ErrorCode::InvalidCloseState
    );
    ...
    require!(
        user_bet.war_id == game_session.war_id_when_played
            && ctx.accounts.war_state.war_id == game_session.war_id_when_played,
    ...
    // The original cycle is already settled and the user's per-cycle PDA was
    // closed by the old war-claim path. Do not try to write leaderboard/HB/MVP
    // score here; the normal path now prevents that ordering for future cycles.
    maybe_run_loser_lootbox_roll(
        claim_won,
        user_bet,
        game_session,
    ...
    if player_data.pending_round_claims > 0 {
        player_data.pending_round_claims = player_data
            .pending_round_claims
            .checked_sub(1)
            .ok_or(ErrorCode::ArithmeticOverflow)?;
...
#[derive(Accounts)]
#[instruction(round_id: u64)]
pub struct LegacyClaimRoundRewards<'info> {
    #[account(
        mut,
        seeds = [PLAYER_DATA_SEED.as_ref(), user_wallet.key().as_ref()],
        bump = player_data.bump,
    ...
    /// CHECK: Correct legacy close-state PDA. Handler requires this account to
    /// be absent/non-program-owned so post-upgrade rounds cannot bypass normal
    /// close counters.
    #[account(
        seeds = [ROUND_CLOSE_STATE_SEED.as_ref(), &round_id.to_le_bytes()],
        bump,
    ...
    /// CHECK: Correct legacy user-bet close-state PDA. Handler requires this
    /// account to be absent/non-program-owned so only sidecar-less legacy bets
    /// can use this rescue path.
    #[account(
        seeds = [USER_GAME_BET_CLOSE_STATE_SEED.as_ref(), user_wallet.key().as_ref(), &round_id.to_le_bytes()],
        bump,
    )]
    ...
    /// CHECK: Original bettor; receives SOL reward and closed bet rent.
    #[account(mut)]
    pub user_wallet: UncheckedAccount<'info>,

    /// Caller paying tx/lootbox-claim rent; may be a cranker.
    #[account(mut)]
    pub caller: Signer<'info>,
    ...
    /// Settled cycle state for the round being rescued.
    #[account(
        seeds = [FACTION_WAR_STATE_SEED, &game_session.war_id_when_played.to_le_bytes()],
        bump = war_state.bump,
    )]
    ...
    /// CHECK: Per-user reservation PDA, same as normal claim path.
    #[account(
        mut,
        seeds = [LOOTBOX_CLAIM_SEED, user_wallet.key().as_ref()],
        bump,
