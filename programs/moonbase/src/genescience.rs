...
pub fn generate_genesis_dna(
    ...
    require!(family_type < 16, crate::errors::ErrorCode::InvalidParameters);
...
pub fn generate_genesis_dna_with_tier(
    ...
    for i in 0..(APPEARANCE_TRAIT_GROUPS * APPEARANCE_TRAITS_PER_GROUP) {
        ...
        set_trait_value(&mut dna, APPEARANCE_TRAITS_OFFSET, APPEARANCE_TRAIT_BITS, i as u8, trait_value);
    }
    ...
    for i in 0..(POWER_TRAIT_GROUPS * POWER_TRAITS_PER_GROUP) {
        ...
        set_trait_value(&mut dna, POWER_TRAITS_OFFSET, POWER_TRAIT_BITS, i as u8, trait_value);
    }
...
fn limit_trait_ranges(dna: &mut [u8; 32]) {
    ...
    for i in 0..(APPEARANCE_TRAIT_GROUPS * APPEARANCE_TRAITS_PER_GROUP) {
        let trait_val = get_trait_value(dna, APPEARANCE_TRAITS_OFFSET, APPEARANCE_TRAIT_BITS, i as u8);
        let limited_val = trait_val % 16; // 0-15
        set_trait_value(dna, APPEARANCE_TRAITS_OFFSET, APPEARANCE_TRAIT_BITS, i as u8, limited_val);
    }
    ...
    for i in 0..(POWER_TRAIT_GROUPS * POWER_TRAITS_PER_GROUP) {
        ...
        set_trait_value(dna, POWER_TRAITS_OFFSET, POWER_TRAIT_BITS, i as u8, limited_val);
    }
}
...
fn set_trait_value(dna: &mut [u8; 32], base_offset: u8, trait_bits: u8, trait_index: u8, value: u8) {
    let bit_offset = base_offset + (trait_index * trait_bits);
    let byte_index = (bit_offset / 8) as usize;
    let bit_in_byte = bit_offset % 8;
    ...
    while bits_remaining > 0 && current_byte_index < 32 {
        ...
        dna[current_byte_index] = (dna[current_byte_index] & !mask) | (value_bits << current_bit_offset);
...
pub fn get_appearance_traits(dna: &[u8; 32]) -> Vec<u8> {
    ...
    for i in 0..(APPEARANCE_TRAIT_GROUPS * APPEARANCE_TRAITS_PER_GROUP) {
        traits.push(get_trait_value(dna, APPEARANCE_TRAITS_OFFSET, APPEARANCE_TRAIT_BITS, i as u8));
    }
    traits
}
...
pub fn get_dominant_appearance_traits(dna: &[u8; 32]) -> Vec<u8> {
    ...
    for i in 0..APPEARANCE_TRAIT_GROUPS {
        ...
        traits.push(get_trait_value(dna, APPEARANCE_TRAITS_OFFSET, APPEARANCE_TRAIT_BITS, trait_index));
    }
    traits
}
...
pub fn get_power_traits(dna: &[u8; 32]) -> Vec<u8> {
    ...
    for i in 0..(POWER_TRAIT_GROUPS * POWER_TRAITS_PER_GROUP) {
        traits.push(get_trait_value(dna, POWER_TRAITS_OFFSET, POWER_TRAIT_BITS, i as u8));
    }
    traits
}
...
pub fn get_dominant_power_traits(dna: &[u8; 32]) -> Vec<u8> {
    ...
    for i in 0..POWER_TRAIT_GROUPS {
        ...
        traits.push(get_trait_value(dna, POWER_TRAITS_OFFSET, POWER_TRAIT_BITS, trait_index));
    }
    traits
}
...
pub fn evolve_dna(dna: &mut [u8; 32], random_seed: &[u8]) -> Result<()> {
    ...
    require!(current_stage < 7, crate::errors::ErrorCode::InvalidParameters);
    ...
    for i in 0..(APPEARANCE_TRAIT_GROUPS * APPEARANCE_TRAITS_PER_GROUP) {
        if random_bytes[i % 32] < 77 { // 77/256 ≈ 30%
            let current = get_trait_value(dna, APPEARANCE_TRAITS_OFFSET, APPEARANCE_TRAIT_BITS, i as u8);
            let max_for_stage = 15 + new_stage; // Increases with evolution
            if current < max_for_stage {
                set_trait_value(dna, APPEARANCE_TRAITS_OFFSET, APPEARANCE_TRAIT_BITS, i as u8, current + 1);
            }
        }
    }
    ...
    for i in 0..(POWER_TRAIT_GROUPS * POWER_TRAITS_PER_GROUP) {
        ...
        if random_bytes[rand_index] < 77 { // 77/256 ≈ 30%
            ...
            if current < max_for_stage {
                set_trait_value(dna, POWER_TRAITS_OFFSET, POWER_TRAIT_BITS, i as u8, current + 1);
            }
        }
    }
...
mod tests {
    ...
    fn test_dna_generation() {
        ...
        for trait_val in appearance {
            assert!(trait_val <= 15, "Appearance trait {} should be <= 15", trait_val);
        }
