...
export async function initializeConfigStores(
  ...
  try {
    ...
    console.error(
      "\x1b[31m%s\x1b[0m",
      "❌ Error initializing config stores:",
      error
    );
    return {
      success: false,
...
export async function addNewModuleToConfigStore(
  ...
  try {
    console.log(
      "\x1b[33m%s\x1b[0m",
      "📡 Adding NEW module config (basic info only)..."
    );
    console.log(
      "\x1b[36m%s\x1b[0m",
      `🔑 Global Config PDA: ${globalConfigPDA.toString()}`
    ...
    console.log(
      "\x1b[36m%s\x1b[0m",
      `🔑 Faction IDs: ${factionIds.join(", ") || "All factions"}`
    );

    // First, get the current next_id from the module config store to derive the PDA
    const moduleConfigStoreAccount =
      await program.account.moduleConfigStore.fetch(moduleConfigStorePDA);
    ...
    console.log(
      "\x1b[36m%s\x1b[0m",
      `🔑 Module Config Account PDA: ${moduleConfigAccountPDA.toString()}`
    );
    ...
    if (moduleType !== "Mining" && moduleType !== "Attraction") {
      console.log(
        "\x1b[33m%s\x1b[0m",
        `⚠️  Module type "${moduleType}" mapped to integer ${moduleTypeInt} (Attraction)`
      );
    }

    // Ensure moduleTypeInt is a number (not string or object)
    const moduleTypeValue =
      typeof moduleTypeInt === "number"
        ? moduleTypeInt
        : parseInt(moduleTypeInt, 10);
    console.log(
      "\x1b[36m%s\x1b[0m",
      `🔑 Module Type Integer: ${moduleTypeValue} (type: ${typeof moduleTypeValue})`
    );

    // Extract only maxHp and powerConsumption from stats for basic config
    const rawStats =
      moduleStats[moduleType.toLowerCase()] ||
      moduleStats[moduleType] ||
      moduleStats;
    const maxHp = rawStats.max_hp ?? rawStats.maxHp;
    const powerConsumption =
      rawStats.power_consumption ?? rawStats.powerConsumption;
    ...
    console.log(
      "\x1b[32m%s\x1b[0m",
      `✅ Module config created (inactive until stats are set)`
    );
    console.log("\x1b[90m%s\x1b[0m", `🔗 Transaction ID: ${configStoreTxid}`);
    console.log(
      "\x1b[90m%s\x1b[0m",
