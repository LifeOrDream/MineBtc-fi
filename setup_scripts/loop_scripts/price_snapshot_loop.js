...
// Load MineBTC IDL
const minebtcIdlPath = path.resolve(
  __dirname,
  config.deployment.paths.minebtc_idl
);
...
const DEGEN_BTC_MINING_SEED = "mine-btc-mining";
...
const raydiumPoolState = new PublicKey(
  deployment.dbtc_sol_pool_created.poolStatePDA
);
const raydiumAmmConfig = new PublicKey(
  deployment.raydium_amm_config_created.amm_config_pda
);
const raydiumAuthority = new PublicKey(
  deployment.dbtc_sol_pool_created.authorityPDA
);
const raydiumObservationState = new PublicKey(
...
const raydiumToken0Vault = new PublicKey(
  deployment.dbtc_sol_pool_created.token0VaultPDA
);
const raydiumToken1Vault = new PublicKey(
  deployment.dbtc_sol_pool_created.token1VaultPDA
);
...
async function runLoop() {
  ...
  while (true) {
    ...
    try {
      ...
      // Step 2: Claim minebtc SOL for distribution
      const claimResult = await executeClaimMinebtcSol();
      if (!claimResult.success) {
        console.log("⚠️  Claim minebtc SOL failed, will retry next iteration");
      }
      ...
      if (priceHistoryLength < 8) {
        ...
        console.log(
          `🔄 Price history = 8, executing update rate and add LP...`
        );
        ...
        console.log(
          `ℹ️  Price history > 8 (${priceHistoryLength}), waiting for next cycle...`
        );
      }
    } catch (error) {
