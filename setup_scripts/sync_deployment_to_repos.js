...
//   2. Writes MineBtcBackend/deployment.json
//   3. Patches mdogeWifBtcFE/hooks/config.ts devnet block
//
// Usage:
...
const cluster = Object.keys(website).find((k) => k !== "pda_seeds") || "devnet";
const w = website[cluster];
if (!w) {
  console.error(`❌ No cluster data found in website.json`);
  process.exit(1);
}
...
function buildBackendDeployment() {
  ...
  const out = {
    [cluster]: {
      ...
      // Sync marker
      synced_from_minebtc_fi_devnet_at: now,
    },
  };

  // Preserve other clusters (mainnet) if they exist
  if (existing.mainnet) out.mainnet = existing.mainnet;
...
function generateFrontendDevnetBlock() {
  ...
  const lines = [
    ...
    `    rpc_url: getReadRpcEndpoint("devnet"),`,
    ...
    `    synced_from_minebtc_fi_devnet_at: ${fmtStr(new Date().toISOString())},`,
    ...
    `    },`,
    `    graphql_endpoint: GRAPHQL_ENDPOINTS.devnet,`,
    `  },`,
  ];
...
function patchFrontendConfig() {
  ...
  // Find the devnet block by brace-counting
  const devnetRegex = /devnet:\s*\{/;
  const match = devnetRegex.exec(original);
  if (!match) {
    console.error("❌ Could not find 'devnet: {' block in hooks/config.ts");
    return;
  }
  ...
  if (endIdx === -1) {
    console.error("❌ Could not find closing brace for devnet block");
    return;
  }

  // We want to replace from 'devnet: {' through the closing '}' (and the
  // trailing comma that separates devnet from the next CONFIGURATION entry).
  ...
  console.log(`✅ Patched frontend hooks/config.ts devnet block`);
}
