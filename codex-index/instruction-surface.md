...
- `sweep_floor_lowest()` — anyone can buy queue.entries[0]; `inventory_sweep_vault` pays SOL while `inventory_pda` receives the NFT, then the contract auto-disposes the asset (push to country lootbox queue if space, else relist at formula markup, else burn if 7-day floor crashed below threshold) and pays a keeper bounty out of `inventory_sweep_vault`.
...
- `inventory_finalize_sale()` — permissionless cleanup; verifies asset's owner is neither `inventory_pda` nor the canonical marketplace escrow PDA, then closes the `RebornEntry`.
...
- `initialize_faction_war(war_id)` - creates the per-cycle war state before that cycle's rounds are folded in.
- `settle_war()` - closes active cycle once linked LP operation target is reached.
- `claim_war_rewards()` - user cycle reward claim.
...
- `settle_round(war_id)` - finalizes reward indexes/pools after winner selection and folds round totals into the active faction-war state.
