...
pub fn buy_module(
    ...
    let electricity_cost = match &module_config.stats {
        ...
        ModuleStats::Attack(stats) => stats.power_consumption as u32,
        ModuleStats::Research(stats) => stats.power_consumption as u32,
    };
...
pub fn claim_research_rewards_internal(
    ctx: Context<ClaimResearchRewards>,
    module_index: u8,
) -> Result<()> {
    let user_moonbase = &mut ctx.accounts.user_moonbase;
    let module_instance = &mut ctx.accounts.module_instance;
    ...
    msg!("🔬 Starting Research rewards claim for user {}", user.key());
    msg!("   Module index: {}", module_index);
    
    // Get the module config from the individual PDA
    let module_config = &module_config_account.data;
    
    // Ensure this is a Research module
    require!(
        module_config.module_type == ModuleType::Research,
        ErrorCode::InvalidModuleType
    );
    
    // Get research stats
    let research_stats = match &module_config.stats {
        ModuleStats::Research(stats) => stats,
        _ => return Err(ErrorCode::ModuleTypeMismatch.into()),
    };
    
    // Get runtime state for Research module
    let (current_hp, current_research_start, research_completed) = match &module_instance.runtime_state {
        ModuleRuntimeState::Research { current_hp, current_research_start, research_completed, .. } => 
            (*current_hp, *current_research_start, *research_completed),
        _ => return Err(ErrorCode::ModuleTypeMismatch.into()),
    };
    
    msg!("✅ Module validation passed - Research module with {} HP", current_hp);
    
    // Check if module is active and has HP
    require!(module_instance.is_active, ErrorCode::ModuleNotActive);
    require!(current_hp > 0, ErrorCode::ModuleDamaged);
    
    // Calculate time since last research start
    let current_time = Clock::get()?.unix_timestamp;
    let time_elapsed = current_time.saturating_sub(current_research_start);
    
    // Get current cooldown based on upgrade level
    let required_cooldown = research_stats.current_cooldown_sec(module_instance.upgrade_level);
    
    msg!("⏰ Research timing:");
    msg!("   Last research start: {}", current_research_start);
    msg!("   Current time: {}", current_time);
    msg!("   Time elapsed: {} seconds", time_elapsed);
    msg!("   Required cooldown: {} seconds", required_cooldown);
    
    // Check if enough time has passed
    if time_elapsed < required_cooldown as i64 {
        let remaining_time = required_cooldown as i64 - time_elapsed;
        msg!("⚠️ Research still in progress - {} seconds remaining", remaining_time);
        return Err(ErrorCode::ResearchNotReady.into());
    }
    
    // Get current stats based on upgrade level
    let success_probability = research_stats.current_probability(module_instance.upgrade_level);
    let max_reward = research_stats.current_max_reward(module_instance.upgrade_level);
    
    // Apply HP efficiency to both probability and reward
    let hp_efficiency = module_instance.hp_efficiency_multiplier(research_stats.max_hp);
    let effective_probability = (success_probability as f64 * hp_efficiency) as u16;
    let effective_max_reward = (max_reward as f64 * hp_efficiency) as u64;
    
    msg!("🎰 Casino Research Roll:");
    msg!("   Base success rate: {:.2}% ({}/10000)", success_probability as f64 / 100.0, success_probability);
    msg!("   HP efficiency: {:.2}%", hp_efficiency * 100.0);
    msg!("   Effective success rate: {:.2}% ({}/10000)", effective_probability as f64 / 100.0, effective_probability);
    msg!("   Max reward: {} mDOGE", effective_max_reward);
    
    // Generate random number for casino roll using clock + user key + module state
    let slot = Clock::get()?.slot;
    let random_seed = anchor_lang::solana_program::keccak::hashv(&[
        &slot.to_le_bytes(),
        &current_time.to_le_bytes(),
        user.key().as_ref(),
        &module_index.to_le_bytes(),
        &research_completed.to_le_bytes(),
    ]);
    
    // Extract random values from hash (professional casino-style)
    let roll_bytes = [random_seed.0[0], random_seed.0[1]];
    let success_roll = u16::from_le_bytes(roll_bytes) % 10000; // 0-9999 (10000 = 100.00%)
    
    let reward_bytes = [random_seed.0[2], random_seed.0[3], random_seed.0[4], random_seed.0[5]];
    let reward_roll = u32::from_le_bytes(reward_bytes);
    
    msg!("🎲 Casino Roll Results:");
    msg!("   Success roll: {} (need < {} to win)", success_roll, effective_probability);
    msg!("   Reward roll: {} (for reward amount)", reward_roll);
    
    // Check if research was successful
    let success = success_roll < effective_probability;
    
    // Update module state regardless of success/failure
    if let ModuleRuntimeState::Research { current_research_start, research_completed, .. } = &mut module_instance.runtime_state {
        *current_research_start = current_time;
        *research_completed = research_completed.saturating_add(1);
    }
    module_instance.last_updated = current_time;
    
    let (reward_amount, reward_description) = if success {
        // Calculate reward amount (10-100% of max reward based on random roll)
        let reward_percentage = 10 + (reward_roll % 91); // 10% to 100%
        let actual_reward = (effective_max_reward * reward_percentage as u64) / 100;
        
        msg!("🎉 RESEARCH SUCCESS!");
        msg!("   Reward percentage: {}%", reward_percentage);
        msg!("   Reward amount: {} mDOGE", actual_reward);
        
        (actual_reward, format!("Research Success ({}%)", reward_percentage))
    } else {
        msg!("💔 Research failed - better luck next time!");
        (0, "Research Failed".to_string())
    };
    
    // Process rewards if successful
    if success && reward_amount > 0 {
        // mDOGE reward - transfer from main custody vault to user
        msg!("💎 Transferring {} mDOGE from custody vault to user", reward_amount);
        ...
        // Transfer from main mDOGE custody vault to user
        anchor_spl::token_2022::transfer_checked(
            CpiContext::new_with_signer(
                ctx.accounts.token_program_2022.to_account_info(),
                anchor_spl::token_2022::TransferChecked {
                    from: ctx.accounts.mdoge_custody_vault.to_account_info(),
                    mint: ctx.accounts.mdoge_token_mint.to_account_info(),
                    to: ctx.accounts.user_mdoge_token_account.to_account_info(),
                    authority: ctx.accounts.mdoge_vault_authority.to_account_info(),
                },
                signer_seeds,
            ),
            reward_amount,
            MDOGE_DECIMALS, // mDOGE decimals
        )?;
        
        msg!("✅ Successfully transferred {} mDOGE from custody vault", reward_amount);
    }
    
    // Award base XP for conducting research (regardless of success)
    let base_research_xp = 25; // Base XP for research attempt
    let bonus_xp = if success { reward_amount / 10_000_000 } else { 0 }; // Bonus XP for successful research
    let total_xp = base_research_xp + bonus_xp as u32;
    ...
    // Emit event
    emit!(ResearchRewardsClaimed {
        owner: user.key(),
        module_index,
        success,
        reward_amount,
        success_probability: effective_probability,
        research_completed: match &module_instance.runtime_state {
            ModuleRuntimeState::Research { research_completed, .. } => *research_completed,
            _ => 0,
        },
        xp_gained: total_xp,
    });
    
    msg!("🎉 Research claim completed!");
    msg!("   User: {}", user.key());
    msg!("   Module Index: {}", module_index);
    msg!("   Success: {}", success);
    if success {
        msg!("   Reward: {} mDOGE ({})", reward_amount, reward_description);
    }
    msg!("   XP Gained: {} (base: {}, bonus: {})", total_xp, base_research_xp, bonus_xp);
    msg!("   Total Research Completed: {}", match &module_instance.runtime_state {
        ModuleRuntimeState::Research { research_completed, .. } => *research_completed,
        _ => 0,
    });
...
pub struct CreateModuleInstance<'info> {
    #[account(
        mut,
        seeds = [USER_MOONBASE_SEED.as_ref(), user.key().as_ref()],
        bump = user_moonbase.bump,
...
#[derive(Accounts)]
#[instruction(module_index: u8)]
pub struct ClaimResearchRewards<'info> {
    #[account(
        mut,
        seeds = [USER_MOONBASE_SEED.as_ref(), user.key().as_ref()],
        bump = user_moonbase.bump,
    ...
    pub mdoge_custody_vault: UncheckedAccount<'info>,

    /// Main mDOGE vault authority
    #[account(
        seeds = [MDOGE_VAULT_AUTHORITY_SEED.as_ref()],
        bump,
    )]
    /// CHECK: Main mDOGE vault authority PDA
    pub mdoge_vault_authority: UncheckedAccount<'info>,

    /// Loot mDOGE vault for XP-based loot transfers
    #[account(
        mut,
        seeds = [LOOT_MDOGE_VAULT_SEED.as_ref()],
        bump,
    ...
    /// Loot mDOGE vault authority for XP-based loot transfers
    #[account(
    ...
    /// User's mDOGE token account for receiving reward tokens (required for mDOGE rewards)
    #[account(mut)]
    /// CHECK: User's mDOGE token account
    pub user_mdoge_token_account: UncheckedAccount<'info>,

    /// mDOGE token mint (Token-2022) (required for mDOGE rewards)
    pub mdoge_token_mint: InterfaceAccount<'info, anchor_spl::token_interface::Mint>,

    /// SPL Token-2022 program (required for mDOGE rewards)
